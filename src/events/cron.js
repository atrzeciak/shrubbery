...
import { upcoming, todayInWarsaw } from "../../public/app/events.js";
...
export async function runDaily(env, now = new Date()) {
  ...
  const events = upcoming(people.results, todayInWarsaw(now), 7).filter((e) => e.inDays === 0 || e.inDays === 7);
  ...
  for (const row of (await q.historySince(db, "event_notice_sent", since).all()).results) {
    if (todayInWarsaw(new Date(row.at * 1000)) !== today) continue;
    let d = {};
